...
#################################################first_come_first_serve#
   def first_come_first_serve(self):
      ...
      while self.number_completed_processes < self.end_condition:
         print("\n\n\n\n") if self.debug is True else None
         print(f"Current simulation clock: {self.cpu.clock}") if self.debug is True else None
         print(f"Number of completed processes: {self.number_completed_processes}") if self.debug is True else None
         ...
         if event.type == "ARR":
            ...
            print("Invalid event type") if self.debug is True else None
         
         print(f"\n\nPost-event processing: Simulation clock: {self.cpu.clock}, Ready queue size: {len(self.ready_queue)}, Disk queue size: {len(self.disk_queue)}") if self.debug is True else None
         ...
         print(f"{'Metrics Report':^40}") if self.debug is True else None
         print(f"{'='*40}") if self.debug is True else None
         print(f"{'Throughput:':<30}{throughput:>10.4f} processes/unit time") if self.debug is True else None      
         print(f"{'CPU Utilization:':<30}{cpu_utilization:>10.4f}%") if self.debug is True else None
         print(f"{'Disk Utilization:':<30}{disk_utilization:>10.4f}%") if self.debug is True else None
         print(f"{'Avg. Turnaround Time:':<30}{avg_turn_around_time:>10.4f} seconds") if self.debug is True else None
         print(f"{'*average_cpu_service_time':<30}{self.total_cpu_service_times / self.end_condition:>10.4f} seconds") if self.debug is True else None
         print(f"{'*average_disk_service_time':<30}{self.total_disk_service_times / self.num_disk_processes:>10.4f} seconds") if self.debug is True else None
         print(f"{'='*40}") if self.debug is True else None
...
##########################################################handleArrival#
   def handleArrival(self, event):
      ...
      if self.cpu.busy is False:
         ...
         if disk_probability <= 0.6:
            # process is done with cpu service
...
######################################################handleDiskArival#
   def handleDiskArival(self, event):
      ...
      if self.disk.busy is False:
         # disk isnt busy
         print("disk is not busy, process Departing Disk after disk service") if self.debug else None
         self.disk.busy = True
         ...
         print("Something is wrong with the disk") if self.debug else None
...
###################################################handleDiskDeparture#
   def handleDiskDeparture(self, event):
      ...
      self.ready_queue.append(event.process)
      print("process appended to ready queue, ", event.process.pid) if self.debug else None
      '''
...
#######################################################handleDeparture#
   def handleDeparture(self, event):
      ...
      if len(self.ready_queue) == 0:
         self.cpu.busy = False
         print("Ready queue is empty. CPU set to idle.\n\n") if self.debug is True else None if self.debug else None
      else:
         # pull the next process from the ready queue
         process_departing = self.ready_queue.pop(0)
         print("process departing", process_departing.pid) if self.debug else None
